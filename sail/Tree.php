...
abstract class Tree {
    ...
    public function run() {
        ...
        if($node == null) {
            throw new NoSuchRouteException('The route ' . $request->getPattern() . ' does not exist');
        }
        ...
        foreach ($node->middleware as $middleware) {
            if (! $middleware instanceof Middleware) {
                throw new \Exception('Object is not instance of class Middleware!');
            }
        
            if (! call_user_func_array(array($middleware, 'call'), $parameters)) {
                $success = false;
            }
        }
    ...
    private function defineRoute($pattern) {
        return $this->retrieveRoute($pattern, function ($waypoint, $temp) {
            $key = $waypoint;
    ...
    private function getNode($pattern) {
        $parameters = array();
        $node = $this->retrieveRoute($pattern, function ($waypoint, $temp) use (&$parameters){
